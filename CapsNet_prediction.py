...
from keras.layers import Dense, Embedding,TimeDistributed,Conv2D,Input, Flatten,Reshape ,MaxPooling2D ,Dropout 
...
print('Build model...')
capsnet = CapsNet(input_shape=[164,148,1],n_class=30 ,routings=3)
capsnet.summary()

model = Sequential()
model.add(TimeDistributed(capsnet,input_shape = (15,164,148,1)))
model.add(Flatten())
...
model.fit(x_train, y_train,
          ...
          callbacks=[ModelCheckpoint('./capsnet_weights3.h5', monitor='val_loss',save_best_only=True, save_weights_only=True,verbose=1)])

model.load_weights('./capsnet_weights3.h5')
print('evaluation.....')
