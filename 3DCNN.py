...
y_train = y_train.reshape((sample_num -timesteps -pre_steps +1, 1,link_num*pre_steps))
...
y_test = y_test.reshape(test_sample_num-timesteps-pre_steps+1,1, link_num*pre_steps)
...
print("train...")
model.fit(x_train, y_train,batch_size = 32,epochs=40,validation_data=(x_test, y_test),callbacks=[ModelCheckpoint('./3dcnn_weights3.h5', monitor='val_loss',save_best_only=True, save_weights_only=True,verbose=1)])
model.load_weights('./3dcnn_weights3.h5')
